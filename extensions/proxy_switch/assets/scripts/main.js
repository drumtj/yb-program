...
function init() {
    ...
    if(manifest.proxy.useCustomProxy){
      console.error("use custom proxy");

      if(manifest.proxy.proxyHttp){
        //////////////////////////////
        profile = {
          "name":"[Untitled Profile]",
          "proxyMode":"manual",
          "proxyHttp": manifest.proxy.proxyHttp, //"124.198.111.32:11959",
          "useSameProxy":true,
          "proxyHttps":"",
          "proxyFtp":"",
          "proxySocks":"",
...
function openMessagePort() {
  chrome.runtime.onConnectExternal.addListener(function (port) {
    if (port.sender.id != 'gpkneknaioihfegiabpoplgdbpibndhj') return;
